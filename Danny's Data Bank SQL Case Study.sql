...
-- Table 1: Regions - Data Bank is also run off a network of nodes where both money and data is stored across the globe. In a traditional banking sense - you can think of these nodes as bank branches or stores that exist around the world.
-- Table 2: Customer Nodes - Customers are randomly distributed across the nodes according to their region - this also specifies exactly which node contains both their cash and data.
-- This random distribution changes frequently to reduce the risk of hackers getting into Data Bank’s system and stealing customer’s money and data!
-- Table 3: Customer Transactions - This table stores all customer deposits, withdrawals and purchases made using their Data Bank debit card.

CREATE DATABASE dannys_databank;
...
#SINCE THE DATA IS HUGE, ANALYSIS PART HAS BEEN WRITTEN EARLIER FOR EASY ACCESS
...
SELECT AVG(DATEDIFF(start_date, prev_date)) AS avg_duration
FROM (
    SELECT customer_id, node_id, start_date,
        LAG(start_date) OVER (PARTITION BY customer_id, node_id ORDER BY start_date) AS prev_date
    FROM customer_nodes
) sq
WHERE prev_date IS NOT NULL;

#By using the LAG function, you can compare the current start date with the previous start date for each customer-node combination and calculate the duration between them using the DATEDIFF function. 
#This helps determine the average duration of customers being reallocated to a different node.
...
HAVING COUNT(DISTINCT CASE WHEN txn_type = 'deposit' THEN customer_id END) > 1
   AND (COUNT(DISTINCT CASE WHEN txn_type = 'purchase' THEN customer_id END) > 0
        OR COUNT(DISTINCT CASE WHEN txn_type = 'withdrawal' THEN customer_id END) > 0)
ORDER BY month;
...
SELECT *, (Deposit - Withdrawal - Purchases) AS Balance
FROM CTE;
